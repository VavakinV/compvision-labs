...
def multi_tracking(video_path):
    ...
    out_ms = cv2.VideoWriter("IZ1/video/output_camshift.mp4",
    ...
    out_csrt = cv2.VideoWriter("IZ1/video/output_csrt.mp4",
                            cv2.VideoWriter_fourcc(*'mp4v'),
                            cap.get(cv2.CAP_PROP_FPS),
                            (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
                                int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    while True:
        ...
        _, track_window_ms = cv2.CamShift(dst, track_window_ms, term_crit)
